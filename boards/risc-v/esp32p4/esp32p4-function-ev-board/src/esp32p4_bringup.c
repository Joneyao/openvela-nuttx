...
static int configure_net(void)
{
  ...
  /* Configure the DNS server: reuse the gateway address (just set
   * above via SIOCSIFDSTADDR) as the DNS server. The company gateway
   * typically forwards DNS queries for public hostnames such as
   * token-plan-cn.xiaomimimo.com. */
  ret = dns_add_nameserver((FAR const struct sockaddr *)&addr,
                           sizeof(struct sockaddr_in));
  printf("[campilot] DNS server set: ret=%d errno=%d\n", ret, errno);
